...
func FileExists(path string) bool {
	_, err := os.Stat(path)
	return !os.IsNotExist(err) // 如果没有错误表示“不存在”，则路径存在
}

func ReadLines(filePath string) ([]string, error) {
	file, err := os.Open(filePath)
	if err != nil {
		return nil, err // 无法打开文件，返回错误
	}
	...
	for scanner.Scan() {
		lines = append(lines, scanner.Text()) // 将读取的每一行添加到切片中
	}

	if err := scanner.Err(); err != nil {
		return nil, err // 处理可能发生的任何错误
	}

	return lines, nil
}

func PreprocessToLowercase(list []string) []string {
	lowerList := make([]string, len(list))
	for i, item := range list {
		lowerList[i] = strings.ToLower(item)
	}
	return lowerList
}
