...
// 定义全局变量存储 homeSite 值
var UniqueHomeSites = make(map[string]bool)
...
func getCompanyInfoById(pid string, isSearch bool, options common.Options, sn *common.Sun) EnInfo {
			...
			if k == "webRecord" {
				...
				for _, y := range t {
					for _, o := range y.Get("domain").Array() {
						...
						// 解析JSON数据
						jsonDataArray := []byte("[" + value + "]")
						var data []map[string]interface{}
						err := json.Unmarshal(jsonDataArray, &data)
						if err != nil {
							fmt.Println("解析JSON数据时出错：", err)
						}

						// 遍历解析后的数据，提取并存储 homeSite 字段的值
						for _, item := range data {
							homeSites, ok := item["homeSite"].([]interface{})
							if !ok {
								fmt.Println("homeSite字段不存在或不是数组")
								continue
							}
							// 遍历homeSite值，进行去重操作
							for _, homeSite := range homeSites {
								if homeSiteStr, ok := homeSite.(string); ok {
									// 如果当前homeSite值不存在于UniqueHomeSites中，则将其记录下来
									if !UniqueHomeSites[homeSiteStr] {
										UniqueHomeSites[homeSiteStr] = true
									}
								}
							}
						}
	...
	if ensInfoMap["invest"].total > 0 && isSearch && options.Invest > 0 {
		...
		for _, t := range enInfo.infos["invest"] {
			...
			// 原50%以上控股递归查询  投资比例这里改成自己可控参数
			if investNum >= app.Setting.Invest {
