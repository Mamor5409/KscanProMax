...
var (
	ceyeApi    = "c68d4dd3c30ecae0626430ff6b2fc506"
	ceyeDomain = "v0fwjn.ceye.io"
)
		...
		go func() {
			for task := range tasks {
				...
				if isVul {
					...
					println("+-------------------------------------------------------------------------------------------------------------------------------------+")
					slog.Println(slog.INFO, "[+] 检测发现存在漏洞，请进行手工核对：")
					red := colorR.BgRed.Render
					format := "| %-30v | %-35v | %s"
					println("+-------------------------------------------------------------------------------------------------------------------------------------+")
					slog.Println(slog.DATA, fmt.Sprintf("\r| %-30v | %-35v | %s ", "URL", "Status", "POC"))
					println("+-------------------------------------------------------------------------------------------------------------------------------------+")
