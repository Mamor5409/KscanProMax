...
//leakscan options：【一坨答辩 先不用了 后面看情况删掉】
//  --leakscan   输入关键字以查询互联网侧的代码泄露、邮箱泄露[暂支持英文]
//   --leakscan-all
//   --leakscan-email
//   --leakscan-code

const usage = "usage: kscan [-h,--help,-v,--version,--fofa-syntax] (-t,--target,-f,--fofa,--spy]) [options] [hydra options] [fofa options] [ascan options] \n\t使用前请在conf.yml下完成配置文件(初次使用可通过-v生成配置文件)"
...
var (
	HostSiteMaps     sync.Map
	HostSiteMapsFofa sync.Map
)
...
func OriginalMain() {
	...
	//检查版本，初次运行自动生成yaml配置文件
	if app.Setting.Version == true {
		//slog.Println(slog.INFO, "初次使用前会检查生成配置文件")
		common.CheckConf()
		...
		if err != nil {
			log.Fatalf("解析 YAML 数据出错: %v", err)
		}
		version := configMap["version"]
		slog.Println(slog.INFO, "当前版本为：", version)
	}
	...
	//ascan模块
	//仅查询单个企业名称相关信息
	if app.Setting.Ascan != "None" && len(app.Setting.AscanFile) == 0 {
		slog.Println(slog.INFO, "ascan模块已开启，开始对输入企业进行信息收集 [请尽可能输入完整的企业名称]")
		//slog.Println(slog.INFO, "可以使用 --ascan-f 参数批量对多个企业进行信息收集")

		if app.Setting.GetBranch == false {
			slog.Println(slog.INFO, "可以使用 --ascan-b 参数开启对开业的分支公司递归查询")
		}
		if app.Setting.Invest == 0 {
			slog.Println(slog.INFO, "可以使用 --ascan-s 参数开启对投资占比[X%]以上的公司递归查询 建议50%以上")
		}

		ascan.Keyword = app.Setting.Ascan
		ascan.Invert = app.Setting.Invest
		ascan.GetBranch = app.Setting.GetBranch
		HostSiteMaps := ascan.Start()

		for domain, _ := range HostSiteMaps {
			println(domain)
			//app.Setting.Target = append(app.Setting.Target, domain)
		}
	}
	// 批量查询 --ascan-f a,b,c,d,e 并扫描
	if len(app.Setting.AscanFile) != 0 && app.Setting.Ascan == "None" {
		...
		for _, value := range CompanyNames {
			...
			for domain := range result {
				HostSiteMaps.Store(domain, struct{}{})
			}
		}

		HostSiteMaps.Range(func(url, value interface{}) bool {
			fmt.Println("[+] 收集到的URL:", url)
			urlString, ok := url.(string)
			if !ok {
				fmt.Println("[-] 无法将 URL 转换为字符串类型")
				return true
			}
			app.Setting.Target = append(app.Setting.Target, urlString)
			//println("[+] 已将以下目标将加入到扫描队列中，请耐心等待......")
			//for _, target := range app.Setting.Target {
			//	println(target)
			//}
			return true
		})
	...
	// ascan联动fofa  --ascan-fofa <xxx公司名> 一般可配合 --check 进行探活扫描加POC探测
	if len(app.Setting.AscanFofa) != 0 && app.Setting.Ascan == "None" {
		var CompanyNames []string
		for index := range app.Setting.AscanFofa {
			CompanyNames = append(CompanyNames, app.Setting.AscanFofa[index])
		}
		for _, value := range CompanyNames {
			app.Setting.Ascan = value
			ascan.Keyword = value
			...
			result := ascan.Start()
			for domain := range result {
				HostSiteMapsFofa.Store(domain, struct{}{})
			}
		}

		HostSiteMapsFofa.Range(func(url, value interface{}) bool {
			fmt.Println("[+] 收集到的URL:", url)
			urlString, ok := url.(string)
			if !ok {
				fmt.Println("[-] 无法将 URL 转换为字符串类型")
			...
			// 如果 URL 以 .gov.cn 结尾，直接加到 Fofa 中
			if strings.HasSuffix(urlString, ".gov.cn") {
				app.Setting.Fofa = append(app.Setting.Fofa, urlString)
				return true
			}
			...
			if len(parts) >= 3 {
				...
				mainDomain = urlString
			}

			// 追加domain到fofa中
			app.Setting.Fofa = append(app.Setting.Fofa, mainDomain)

			return true
		})
	}

	//敏感信息获取模块  github gitlab searchcode | email  效果依托答辩
	if app.Setting.Leakscan != "None" {
		leakscan.Keyword = app.Setting.Leakscan
		leakscan.EmailScan = app.Setting.Emailscan
		leakscan.CodeScan = app.Setting.Codescan

		slog.Println(slog.INFO, "leakscan模块已开启，开始对输入信息进行敏感信息收集 [目前只支持英文、准确性尚不能保证需手动核查:) ]")

		//println("[-] leakscan-eamil bool", app.Setting.Emailscan)
		//println("[-] leakscan-code bool", app.Setting.Codescan)
		//println("[-] leakscan-all bool", app.Setting.Leakscanall)

		if app.Setting.Emailscan == false {
			slog.Println(slog.INFO, "可以使用 --leakscan-email 参数只收集相关邮箱信息")
		}
		if app.Setting.Codescan == false {
			slog.Println(slog.INFO, "可以使用 --leakscan-cide 参数只收集泄露代码信息")
		}

		if app.Setting.Leakscanall == false {
			//slog.Println(slog.INFO, "可以使用 --leakscan-all 参数收集泄露代码及邮箱信息")

			if app.Setting.Emailscan == true {
				slog.Println(slog.INFO, "收集相关邮箱泄露信息......")

				emails := leakscan.SearchSkymem(leakscan.Keyword)
				emailsjsonStr, err := json.Marshal(emails)
				if err != nil {
					fmt.Println("Error:", err)
					return
				}
				fmt.Println(string(emailsjsonStr))
			}
			if app.Setting.Codescan == true {
				slog.Println(slog.INFO, "收集相关代码泄露信息......")

				codes := leakscan.SearchBySearchcodeApi(leakscan.Keyword)
				codesjsonStr, err := json.Marshal(codes)
				if err != nil {
					fmt.Println("Error:", err)
					return
				}
				fmt.Println(string(codesjsonStr))
			}
		}
		if app.Setting.Leakscan != "None" {
			slog.Println(slog.INFO, "收集相关邮箱泄露信息......")
	...
	// 渲染文件结尾并整合写入Html文件
	if app.Setting.OutputHtml != nil {
		// 保存为html
		app.HTMLout()
		app.WriteToHtml(app.Args.OutputHtml)
	}
...
func Init() {
	...
	//配置文件初始化
	app.ConfigInit()
	//html 输出结果初始化,poc检测结果,url检测结果，端口扫描结果
...
const (
	...
	htmltemplatePath = "static/template.html"
)
...
func InitFofa() {
	...
	CfgYFile, err := ioutil.ReadFile("./conf.yml")
	if err != nil {
		log.Fatalf("[-] 无法读取配置文件: %v", err)
	}
	configMap := make(map[string]string)
	// 解析 YAML 数据到 map 中
	err = yaml.Unmarshal(CfgYFile, &configMap)
	if err != nil {
		log.Fatalf("[-] 解析 YAML 数据出错: %v", err)
	}

	email := configMap["FOFA_EMAIL"]
	key := configMap["FOFA_KEY"]
