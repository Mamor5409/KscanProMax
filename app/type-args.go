...
type args struct {
	...
	OutputJson, OutputCSV, OutputHtml               string
	Spy, Touch                                      string
...
func (o *args) define() {
	...
	//verision config
	sflag.BoolVar(&o.Version, "v", false)
	sflag.BoolVar(&o.Version, "version", false)

	//ascan模块
	sflag.AutoVarString(&o.ascan, "ascan", "None")
	sflag.Float64Var(&o.invest, "ascan-s", 0)
	sflag.BoolVar(&o.getbranch, "ascan-b", false)
	sflag.StringSpliceVar(&o.ascanfile, "ascan-f")
	sflag.StringSpliceVar(&o.ascanfofa, "ascan-fofa")

	//leakscan模块
	sflag.AutoVarString(&o.leakscan, "leakscan", "None")
	sflag.BoolVar(&o.leakscanall, "leakscan-all", false)
	sflag.BoolVar(&o.emailscan, "leakscan-email", false)
	sflag.BoolVar(&o.codescan, "leakscan-code", false)

	//nopoc参数
	sflag.BoolVar(&o.nopoc, "nopoc", false)

	//exploit模块
	sflag.AutoVarString(&o.exploit, "exploit", "None")
	...
	//新加一个html输出模块
	sflag.StringVar(&o.OutputHtml, "oH", "")
...
func (o *args) CheckArgs() {
	//判断必须的参数是否存在
	if len(o.Target) == 0 && len(o.Fofa) == 0 && o.Spy == "None" && len(o.ascan) == 0 && o.DownloadQQwry == false {
		fmt.Print("至少有--target、--fofa、--spy、--ascan参数中的一个")
		os.Exit(0)
	}
	//判断冲突参数
	if len(o.Target) > 0 && len(o.Fofa) == 0 && o.Spy != "None" && o.Touch == "None" {
		fmt.Print("--target、--fofa、--spy、--ascan不能同时使用")
		os.Exit(0)
	}
