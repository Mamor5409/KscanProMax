...
type Config struct {
	...
	//版本
	Version bool
	//ascan aqc模块    deep 深度默认为2 如果加上参数表示开启
	Ascan     string   // 默认为 -n参数 单个公司查询
	Invest    float64  // ascan -s参数 深度递归查询
	GetBranch bool     // ascan -b参数 递归查询子公司
	AscanFile []string // 批量导入待查询企业名
	AscanFofa []string // 联动fofa
	...
	// 爆破模块完善 http  比如tomcat(auth认证类)    phpmyadmin jenkins。。。
...
func ConfigInit() {
	...
	Setting.OutputHtml = loadOutputHtml(args.OutputHtml)
	...
	//ascan模块
	Setting.Ascan = args.ascan
	Setting.Invest = args.invest
	Setting.GetBranch = args.getbranch
	Setting.AscanFile = args.ascanfile
	Setting.AscanFofa = args.ascanfofa

	////exploit模块
	//Setting.Exploit = args.exploit

	//leakscan模块
	Setting.Leakscan = args.leakscan
	Setting.Leakscanall = args.leakscanall
	Setting.Emailscan = args.emailscan
	Setting.Codescan = args.codescan

	//nopoc 参数
	Setting.Nopoc = args.nopoc
...
func loadOutputHtml(path string) *HTMLWriter {
	if !misc.FileExists("./static/template.html") {
		slog.Println(slog.INFO, "html模板不存在，禁用html导出功能")
		return nil
	}
	if path == "" {
		return nil
	...
	if _, err := os.Stat(path); err == nil || os.IsExist(err) {
		slog.Println(slog.WARN, "检测到Html输出文件已存在，将自动删除该文件：", path)
		if err := os.Remove(path); err != nil {
			slog.Println(slog.ERROR, "删除文件失败，请检查：", err)
		}
	}
	...
	if len(Args.Target) != 0 {
		headtitle = strings.Join(Args.Target, ", ")
	} else if len(Args.ascanfofa) != 0 {
		headtitle = strings.Join(Args.ascanfofa, ", ")
	} else if len(Args.Fofa) != 0 {
		headtitle = strings.Join(Args.Fofa, ", ")
	}
	var tmpscope []string
	if Args.Check {
		tmpscope = append(tmpscope, "URL检测")
	} else if Args.Scan {
		tmpscope = append(tmpscope, "端口扫描")
	}
	if Args.Hydra {
		tmpscope = append(tmpscope, "弱口令检测")
	}
	if !Args.nopoc {
		tmpscope = append(tmpscope, "漏洞检测")
	}
	Scope := strings.Join(tmpscope, ", ")
	...
	ReadTemplate(data)
	return &HTMLWriter{HTMLBuffer, &sync.Mutex{}}
}
